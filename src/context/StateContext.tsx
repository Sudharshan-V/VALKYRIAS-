...
interface StateContextType {
  ...
  login: (role: 'admin' | 'client' | 'editor', token: string, user: any) => void;
  logout: () => void;
  projects: Project[];
...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadUserData = async (userId: string) => {
    try {
      ...
      const springActionItems = await api.fetchActionItems(userId);
      if (springActionItems && springActionItems.length > 0) {
        setActionItemsInternal(springActionItems);
      }
      const springChats = await api.fetchChatMessages(userId);
      if (springChats && springChats.length > 0) {
        setChatMessages(springChats);
      }
      const springSettings = await api.fetchSettings(userId);
      if (springSettings) {
        setTotalContract(springSettings.totalContract ?? 1240000);
        setPaidToDate(springSettings.paidToDate ?? 790000);
        setNextInvoice(springSettings.nextInvoice ?? 450000);
        setActivePlan(springSettings.activePlan);
        setStorageUsed(Number(springSettings.storageUsed) || 1.2);
        setStorageTotal(Number(springSettings.storageTotal) || 2.0);
      }
      const springDeliverables = await api.fetchDeliverables(userId);
      if (springDeliverables && springDeliverables.length > 0) {
        setDeliverables(springDeliverables);
      }
      const springPortfolio = await api.fetchPortfolioItems(userId);
      if (springPortfolio && springPortfolio.length > 0) {
        setPortfolioItems(springPortfolio);
      }
      const springPlans = await api.fetchPlans(userId);
      if (springPlans && springPlans.length > 0) {
        setPlans(springPlans);
      }
    } catch (error) {
      console.error("Error loading user data from Spring Boot:", error);
    }
  };
  ...
  const setProjects: React.Dispatch<React.SetStateAction<Project[]>> = (value) => {
    setProjectsInternal((prev) => {
      ...
      const userJson = localStorage.getItem('valkyrias_user');
      if (userJson) {
  ...
  const login = (role: 'admin' | 'client' | 'editor', token: string, user: any) => {
    localStorage.setItem('valkyrias_jwt_token', token);
    localStorage.setItem('valkyrias_user', JSON.stringify(user));
  ...
  const logout = () => {
    localStorage.removeItem('valkyrias_jwt_token');
    localStorage.removeItem('valkyrias_user');
  ...
  const updatePlan = async (id: string, updated: Partial<Plan>) => {
    ...
    if (userJson) {
      ...
      await api.savePlan({
        ...
        desc: fullUpdated.desc,
        features: fullUpdated.features,
        isPopular: fullUpdated.isPopular
      });
    }
  };
  ...
  const resolveActionItem = async (id: string, action: 'resolved' | 'accepted') => {
    ...
    if (userJson) {
      ...
      if (foundItem) {
        ...
        if (action === 'accepted' && foundItem.type === 'order') {
  ...
  const deleteNote = async (id: string) => {
    ...
    await api.deleteNoteFromApi(id);
  };
