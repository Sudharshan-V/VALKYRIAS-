...
import { Project, ActionItem, ChatMessage, Deliverable, PortfolioItem, ActiveView, Plan, Note, ProfileResponse } from '../types';
...
export const AppStateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const profileHydrationRef = useRef<{ userId: string; promise: Promise<void> } | null>(null);
  ...
  const routeForRole = (role: ProfileResponse['role']): ActiveView => {
    if (role === 'ADMIN') return 'admin';
    if (role === 'EDITOR') return 'client';
    return 'customer';
  };

  const refreshProfile = async (): Promise<ProfileResponse> => {
    const loadedProfile = await api.getMyProfile();
    setProfile(loadedProfile);
    return loadedProfile;
  };

  const establishAuthenticatedSession = (userId: string): Promise<void> => {
    ...
    const promise = (async () => {
      try {
        ...
        setProfile(loadedProfile);
        const mappedRole = loadedProfile.role.toLowerCase() as 'admin' | 'client' | 'editor';
        setLoggedInUser(mappedRole);
        setView((currentView) => (
          currentView === 'login' || currentView === 'landing'
            ? routeForRole(loadedProfile.role)
            : currentView
        ));
        ...
        if (session?.user.id === userId) {
          // A valid Supabase session is not enough to authorize a dashboard
          // role. Fail closed until Spring Boot verifies the token and role.
          console.error('Unable to verify the authenticated profile with Spring Boot:', error);
          setProfile(null);
    ...
    profileHydrationRef.current = { userId, promise };
    promise.then(
      () => {
  ...
  // Dashboard authorization always comes from the Spring Boot profile response.
  useEffect(() => {
    const initSession = async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (session) {
        await establishAuthenticatedSession(session.user.id).catch(() => undefined);
      }
    };
    void initSession();
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      if (session) {
        // Supabase recommends keeping this callback synchronous. Defer API work
        // until after its internal auth-state lock has been released.
        window.setTimeout(() => {
          void establishAuthenticatedSession(session.user.id).catch(() => undefined);
        }, 0);
      } else {
        setLoggedInUser(null);
        resetToDefaults();
        setView((currentView) => (
          ['admin', 'client', 'customer', 'checkout'].includes(currentView)
            ? 'login'
            : currentView
        ));
      }
  ...
  // Resolve the current role from the authenticated backend. The role selected
  // on the login screen is only a portal hint and is never authorization data.
  const login = async () => {
    const { data: { session }, error } = await supabase.auth.getSession();
    if (error || !session) {
      throw error ?? new Error('No authenticated session is available.');
    }
  ...
  const logout = async () => {
    ...
    // Remove the obsolete pre-profile role hint from older browser sessions.
