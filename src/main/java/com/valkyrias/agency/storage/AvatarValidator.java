...
public class AvatarValidator {
    ...
    public AvatarValidator(
            @Value("${supabase.storage.max-avatar-bytes:5242880}") long maxBytes
    ) {
        if (maxBytes <= 0) {
            throw new IllegalArgumentException(
                    "Avatar size limit must be positive"
            );
        }
    ...
    public ValidatedAvatar validate(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new AvatarValidationException(
                    "Avatar file is required and must not be empty"
            );
        }
        ...
        /*
         * Detect the real image format from the file content.
         * Do not trust or compare the browser-provided MIME type,
         * because some browsers and Windows files report it incorrectly.
         */
