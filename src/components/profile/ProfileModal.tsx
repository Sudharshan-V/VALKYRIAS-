...
function friendlyError(error: unknown): string {
  if (error instanceof ApiError) {
    if (error.status === 401) {
      return 'Your session has expired. Sign in again to view your profile.';
    }

    if (error.status === 403) {
      return 'Your account is not permitted to access this profile.';
    }
...
const ProfilePanel: React.FC<ProfilePanelProps> = ({
  ...
  const handleSaved = async (updated: ProfileResponse) => {
    ...
    if (!refreshed && mountedRef.current) {
      ...
      setLoadError('');
      setSuccessMessage(
        'Profile saved. Fresh data will be loaded the next time you open it.'
      );
    }
  };

  const handleAvatarChanged = async (
    updated: ProfileResponse | null,
    message: string
  ) => {
    if (updated) {
      setProfile(updated);
  ...
  return (
    <div
      className="
        ...
        rounded-[28px]
        border
        border-primary-gold/15
        bg-surface-container
        shadow-[20px_20px_60px_rgba(0,0,0,0.8)]
        sm:h-[calc(100dvh-3rem)]
      "
    >
      <div className="flex shrink-0 items-center justify-between gap-4 border-b border-white/5 bg-surface-container-low/85 px-5 py-4 backdrop-blur-xl sm:px-7">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <h2 className="truncate font-display text-xl font-black text-white">
              My profile
            </h2>
          </div>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-5 py-5 sm:px-7 sm:py-6">
        {loading && !profile ? (
          <ValkyriasLoader
            ...
            detail="Authorizing private identity data"
          ...
          <div className="space-y-6">
            {successMessage && (
              <div
                role="status"
                className="flex items-center justify-between gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.07] p-3 text-xs text-emerald-300"
              >
            ...
            {loadError && (
              <div
                role="alert"
                className="flex items-start gap-2 rounded-xl border border-amber-400/20 bg-amber-400/[0.05] p-3 text-xs text-amber-400"
              >
              ...
              <div className="min-w-0">
                ...
                <p className="mt-1 text-[10px] text-gray-500">
                  {editing
                    ...
                    : 'Only you can edit this private profile.'}
                </p>
              </div>
