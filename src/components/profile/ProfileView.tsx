...
export function isProfileIncomplete(profile: ProfileResponse): boolean {
...
export const ProfileView: React.FC<ProfileViewProps> = ({ profile }) => (
  <div className="space-y-7">
    {isProfileIncomplete(profile) && (
      <div className="flex items-start gap-3 rounded-xl border border-amber-400/20 bg-amber-400/[0.06] p-4 text-xs text-amber-100">
        <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-300" />
        <div>
          <p className="font-bold">Your profile is incomplete</p>
          <p className="mt-1 leading-relaxed text-amber-100/65">Add the missing details so your dashboard and collaborator identity stay current.</p>
        </div>
      </div>
    )}
