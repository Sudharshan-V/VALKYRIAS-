...
import {
  ...
  UploadCloud, FileUp, Briefcase, ShieldCheck,
  ArrowLeft
} from 'lucide-react';
...
const ClientDashboardContent: React.FC = () => {
  ...
  const [uploadCategory, setUploadCategory] = useState<Extract<FileCategory, 'PREVIEW' | 'DELIVERABLE'>>('PREVIEW');
  ...
  const [deleteTarget, setDeleteTarget] = useState<{
    ...
    filename: string;
    storagePath?: string;
    source: 'deliverable' | 'client asset';
  } | null>(null);
  ...
  const activeChatMessages = chatMessages.filter(msg => msg.projectId === selectedProjectId);
  ...
  const projectDeliverables = deliverables.filter(item => item.projectId === selectedProjectId);

  // Split deliverables into editor-produced assets and client-uploaded reference files
  const clientUploadedAssets = projectDeliverables.filter(item => item.category === 'CLIENT_ASSET');

  const editorDeliverables = projectDeliverables.filter(item => item.category === 'PREVIEW' || item.category === 'DELIVERABLE');

  const handleDownload = async (item: any) => {
    const download = await getFileDownload(item.id);
  ...
  const startUpload = async (file: File) => {
    ...
    try {
      ...
      const uploadedItem = await uploadDeliverable(file, selectedProjectId, uploadCategory, setUploadProgress);
      setUploadingState('verifying');

      if (uploadedItem && uploadCategory === 'PREVIEW') {
        await orderApi.markPreviewReady(selectedProjectId);
        await refreshData();
        addChatMessage(
          'editor',
          `Preview uploaded and sent for client review: ${file.name} (${uploadedItem.size})`,
          selectedProjectId,
        );
      } else if (uploadedItem) {
        addChatMessage(
          'editor',
          `Published new production deliverable: ${file.name} (${uploadedItem.size})`,
          selectedProjectId,
        );
      }
    ...
    <div className="min-h-screen bg-obsidian text-gray-200 font-sans p-6 md:p-12 relative overflow-hidden">
              ...
              >
                ...
                <div className="flex items-center space-x-4 mb-4">
                  <MediaThumbnail
                    ...
                    alt={proj.title}
                    fallback="project"
                    className="w-12 h-12 rounded-xl flex-shrink-0 border border-white/10 group-hover:scale-105 transition-transform duration-300"
                  />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="neumorphic-flat p-6 rounded-3xl space-y-5 border border-primary-gold/10">
            ...
            <div className="flex flex-wrap gap-3 border-t border-white/5 pt-4">
              {selectedProject.orderStatus === 'EDITOR_ASSIGNED' && (
                <>
                  <button disabled={workflowBusy} onClick={() => void runWorkflowAction(() => orderApi.respondToAssignment(selectedProject.id, true))} className="px-4 py-2 rounded-xl bg-primary-gold text-obsidian text-[10px] font-mono font-bold disabled:opacity-50 cursor-pointer">ACCEPT ASSIGNMENT</button>
                  <button disabled={workflowBusy} onClick={() => void runWorkflowAction(() => orderApi.respondToAssignment(selectedProject.id, false))} className="px-4 py-2 rounded-xl border border-red-500/30 text-red-400 text-[10px] font-mono font-bold disabled:opacity-50 cursor-pointer">REJECT ASSIGNMENT</button>
                </>
              )}
              {selectedProject.orderStatus === 'ACCEPTED' && (
                <button disabled={workflowBusy} onClick={() => void runWorkflowAction(() => orderApi.startOrder(selectedProject.id))} className="px-4 py-2 rounded-xl bg-primary-gold text-obsidian text-[10px] font-mono font-bold disabled:opacity-50 cursor-pointer">START WORK</button>
              )}
              {(selectedProject.orderStatus === 'IN_PROGRESS' || selectedProject.orderStatus === 'REVISION_REQUESTED') && (
                <button disabled={workflowBusy} onClick={() => void runWorkflowAction(() => orderApi.markPreviewReady(selectedProject.id))} className="px-4 py-2 rounded-xl bg-primary-gold text-obsidian text-[10px] font-mono font-bold disabled:opacity-50 cursor-pointer">MARK PREVIEW READY</button>
              )}
              {selectedProject.orderStatus === 'PAID' && (
                <button disabled={workflowBusy} onClick={() => void runWorkflowAction(() => orderApi.deliverOrder(selectedProject.id))} className="px-4 py-2 rounded-xl bg-primary-gold text-obsidian text-[10px] font-mono font-bold disabled:opacity-50 cursor-pointer">MARK DELIVERED</button>
              )}
              <span className="px-3 py-2 rounded-xl border border-white/10 text-gray-400 text-[10px] font-mono">{selectedProject.orderStatus?.replaceAll('_', ' ')}</span>
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              <div className="flex items-center gap-4">
                <input
                  ...
                  min="0"
                  max="100"
                  value={Number.isFinite(Number(selectedProject.progress)) ? Number(selectedProject.progress) : 0}
                  onChange={(e) => updateProjectProgress(selectedProject.id, Number(e.target.value))}
                  disabled={selectedProject.orderStatus !== 'IN_PROGRESS' && selectedProject.orderStatus !== 'REVISION_REQUESTED'}
                  className="render-progress-slider flex-1 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50"
                  style={{ '--progress': `${Number.isFinite(Number(selectedProject.progress)) ? Number(selectedProject.progress) : 0}%` } as React.CSSProperties}
                />
              </div>
            </div>
          </div>
          ...
          <div className="neumorphic-flat p-6 rounded-3xl space-y-4">
            ...
            <label className="space-y-2 block">
              <span className="font-mono text-[9px] text-gray-500 uppercase tracking-wider">FILE WORKFLOW CATEGORY</span>
              <select value={uploadCategory} onChange={(event) => setUploadCategory(event.target.value as Extract<FileCategory, 'PREVIEW' | 'DELIVERABLE'>)} className="neu-select w-full px-4 py-3 rounded-xl text-xs text-white">
                <option value="PREVIEW">Preview for client review</option>
                <option value="DELIVERABLE">Final deliverable (verified payment required)</option>
              </select>
            </label>
          ...
          <div className="neumorphic-flat p-6 rounded-3xl space-y-4">
            ...
            {editorDeliverables.length > 0 ? (
                ...
                {editorDeliverables.map((item) => (
                  <div key={item.id} className="p-4 rounded-xl bg-obsidian border border-white/5 flex items-center space-x-4">
                    <MediaThumbnail
                      ...
                      alt={item.filename}
                      fallback="file"
                      className="w-12 h-12 rounded-lg flex-shrink-0 border border-white/10"
                    />
              ...
              <div className="p-6 text-center text-gray-500 border border-white/[0.03] rounded-2xl bg-obsidian/30 font-mono text-xs">
          ...
          <div className="neumorphic-flat p-6 rounded-3xl space-y-4 border border-primary-gold/10 bg-gradient-to-r from-obsidian/60 via-surface-container/60 to-obsidian/60">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              <div className="flex items-center space-x-2">
            ...
            {clientUploadedAssets.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {clientUploadedAssets.map((item) => {
                  const ext = safeLower(item.filename).split('.').pop() || '';
              ...
              <div className="p-8 text-center text-gray-500 border border-dashed border-white/10 rounded-2xl bg-obsidian/20 font-mono text-xs">
        ...
        <div className="lg:col-span-5 neumorphic-flat p-6 rounded-3xl flex flex-col justify-between h-[650px] border border-white/5">
          ...
          {/* Messages Area */}
          <div ref={chatScrollRef} className="flex-1 overflow-y-auto py-4 space-y-4 px-1 my-2 scrollbar-none overscroll-contain">
            {activeChatMessages.length > 0 ? (
              activeChatMessages.map((msg) => {
                const isOwnMessage = msg.sender === 'editor';
                  ...
                  >
                    {!isOwnMessage && (
                      <ProfileAvatar
                        src={avatarUrl}
                        name={msg.senderName}
                        className="h-8 w-8 rounded-full border border-white/10 bg-obsidian text-[9px] font-black"
                      />
                    )}
                    <div className={`flex max-w-[80%] flex-col ${isOwnMessage ? 'items-end' : 'items-start'}`}>
                      <div className={`rounded-2xl p-3.5 text-xs font-sans ${
                        isOwnMessage
                      ...
                      <span className="mt-1 px-1 font-mono text-[8px] text-gray-500">{msg.senderName} • {msg.time}</span>
                    </div>
                    {isOwnMessage && (
                      <ProfileAvatar
                        src={avatarUrl}
                        name={msg.senderName}
                        className="h-8 w-8 rounded-full border border-primary-gold/20 bg-obsidian text-[9px] font-black"
                      />
                    )}
                  </div>
                );
      ...
      <ConfirmDialog
        ...
        title={`Delete ${deleteTarget?.source || 'file'} permanently?`}
        description={(
          <>
            <span className="font-semibold text-white">{deleteTarget?.filename}</span>
            {' '}will be removed from secure storage and the project database. This action cannot be undone.
          </>
        )}
...
export const ClientDashboard: React.FC = () => {
  const { projects, dataLoading, dataError, refreshData } = useAppState();
  if (dataLoading) {
    return <ValkyriasLoader fullPage label="Loading editor portal" detail="Synchronizing assigned projects and secure files" />;
  }
  if (dataError) {
  ...
  if (projects.length === 0) {
    return <div className="min-h-screen bg-obsidian flex flex-col items-center justify-center gap-3 text-gray-400"><Briefcase className="w-8 h-8 text-primary-gold" /><p className="font-mono text-xs">NO ASSIGNED ORDERS YET</p></div>;
  }
