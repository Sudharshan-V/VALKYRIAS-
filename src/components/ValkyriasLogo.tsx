...
export const ValkyriasLogo: React.FC<ValkyriasLogoProps> = ({
  ...
  return (
    <div className={`flex ${centered ? 'flex-col items-center text-center' : 'items-center space-x-3'} group cursor-pointer select-none`}>
      ...
      <div className={`relative ${currentSize.container} flex-shrink-0`}>
        ...
        <div className="absolute inset-0 bg-primary-gold/25 rounded-full blur-[10px] scale-95 group-hover:scale-110 transition-transform duration-500 animate-pulse" />
        
        <svg
          className="w-full h-full text-primary-gold filter drop-shadow-[0_0_8px_rgba(223,178,113,0.75)] drop-shadow-[0_0_16px_rgba(223,178,113,0.4)] transition-transform duration-500 group-hover:scale-105"
          viewBox="0 0 100 100"
          fill="none"
      ...
      {showText && (
        <div className={`flex flex-col relative overflow-hidden py-1 pr-2 ${centered ? 'mt-4 items-center' : ''}`}>
          <h1 className={`font-display font-extrabold tracking-[0.2em] text-transparent bg-clip-text bg-gradient-to-r from-gray-400 via-gray-100 to-gray-500 uppercase relative select-none leading-none ${currentSize.textTitle}`}>
            VALKYRIAS
          ...
          <span className={`font-mono tracking-[0.4em] text-gray-500 uppercase mt-1.5 relative leading-none ${currentSize.textSub}`}>
            SINCE 2022
